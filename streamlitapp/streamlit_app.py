...
from streamlitapp.side_bar import SideBar
import streamlit as st
from streamlit_feedback import streamlit_feedback
from streamlitapp.style.app_markdown import MARKDOWN_STR
from streamlitapp.utils.local_chat_history import LocalChatMessageHistory
from streamlitapp.utils.message_editing import MessageEditing
from streamlitapp.utils.multimodal_utils import format_content, get_parts_from_files
from streamlitapp.utils.stream_handler import Client, StreamHandler, get_chain_response
...
def handle_user_input(side_bar: SideBar) -> None:
    ...
    if prompt:
        st.session_state.modified_prompt = None
        parts = get_parts_from_files(uploaded_files=side_bar.uploaded_files)
        st.session_state["gcs_uris_to_be_sent"] = ""
        ...
        generate_ai_response(url_input_field=side_bar.url_input_field)
        update_chat_title()
        if len(parts) > 1:
...
def generate_ai_response(url_input_field: str) -> None:
    ...
    with ai_message:
        ...
        client = Client(url=url_input_field)
...
def display_feedback(side_bar: SideBar) -> None:
    ...
    if st.session_state.run_id is not None:
        ...
        if feedback is not None:
            client = Client(url=side_bar.url_input_field)
            client.log_feedback(
